...
//        fansCount.setText(Utility.convertStateNumberToString(getActivity(), userBean.getFollowers_count()));
        userBean.setFollowers_count("999");
        fansCount.setText(Utility.convertStateNumberToString(getActivity(),"38"));
...
//        TimeLineBitmapDownloader.getInstance().downloadAvatar(avatar.getImageView(), userBean, (AbstractTimeLineFragment) this);
        ...
        if (!TextUtils.isEmpty(userBean.getDescription())) {
            bio.setText(userBean.getDescription());//个性签名？yangxj@20131018
        ...
        if (!TextUtils.isEmpty(userBean.getUrl())) {//什么路径啊？yangxj@20131018
...
//        final int height = viewPager.getHeight();
    ...
    class HeaderPagerAdapter extends PagerAdapter {
        ...
        @Override
        public int getCount() {
            return userBean.isVerified() ? 3 : 2;//认证用户？yangxj@20131018
        }
